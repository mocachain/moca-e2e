...
if [ "$ENV" != "local" ]; then echo "SKIP: SP params test only on local"; exit 0; fi
...
STORAGE_PARAMS=$(exec_mocad query storage params --node tcp://localhost:26657 --output json 2>/dev/null || echo "")
...
PAYMENT_PARAMS=$(exec_mocad query payment params --node tcp://localhost:26657 --output json 2>/dev/null || echo "")
...
VG_PARAMS=$(exec_mocad query virtualgroup params --node tcp://localhost:26657 --output json 2>/dev/null || echo "")
