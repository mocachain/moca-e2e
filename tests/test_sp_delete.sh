...
PRE="$(exec_mocad query sp storage-provider-by-operator-address "$OP" --node "$TM_RPC" --output json 2>/dev/null || echo "")"
...
GOV="$(exec_mocad query gov proposals --node "$TM_RPC" --output json 2>/dev/null || echo "")"
